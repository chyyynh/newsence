 ...
 * Works by extracting caption data from the video page
 */
export async function fetchYouTubeTranscript(videoId: string): Promise<TranscriptSegment[]> {
	...
	if (captionTracks.length === 0) {
		console.log('[YOUTUBE-TRANSCRIPT] No captions available');
		return [];
	}
	...
	console.log(`[YOUTUBE-TRANSCRIPT] Using caption track: ${track.languageCode}`);
