...
export async function handleYouTubeCron(
	env: Env,
	_ctx: ExecutionContext,
): Promise<void> {
	...
	try {
		...
		for (const channel of channels) {
			...
			try {
				...
				const entries: YouTubeAtomEntry[] = rawEntries
				...
				const existing = await db.query(
					`SELECT url FROM ${table} WHERE url = ANY($1)`,
					[videoUrls],
				);
				const existingSet = new Set((existing.rows as { url: string }[]).map((r) => normalizeUrl(r.url)));

				const newEntries = entries.filter(
					(e) => !existingSet.has(normalizeUrl(`https://www.youtube.com/watch?v=${e["yt:videoId"]}`)),
				);
