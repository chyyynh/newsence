 ...
 *
 * The R2 write + DB row commit helpers live here too, because all blob paths
 * need the same compensating cleanup if the database commit fails.
 */

import { type InsertBlobUserFileData, insertBlobUserFile, withDbTransaction } from '@shared/db';
import {
	extensionFromMime,
	isRasterImage,
	MAGIC_SNIFF_BYTES,
