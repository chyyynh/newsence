// Hybrid pgvector + keyword search with RRF + recency decay. SQL kept
// verbatim against frontend/src/lib/search/index.ts so the worker chat and
// the UI search bar return the same ranking. Worker-side deltas:
//   - raw pg client.query in place of Prisma
//   - env.AI.run('@cf/baai/bge-m3') in place of /embed self-fetch
//   - no LRU cache (the Workers isolate is too short-lived to benefit)

import type { Client } from 'pg';
import { generateArticleEmbedding } from '../../infra/embedding';
import type { Env } from '../../models/types';

export type SearchResponse = Map<string, number>;

// Shared SELECT projections for the `articles` table. Summary is what
// search-news returns to the model; Full adds body content for read-context.
// Single source of truth so a column rename touches one place.
